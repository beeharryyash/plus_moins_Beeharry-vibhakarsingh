...
void plus_moins(int choix_joueur, int valeur_a_trouver)  //compare le choix_joueur et valeur_a_trouver
{
...
int difficulty(int niveau_diff)   // modes de difficulte
{
   switch(niveau_diff)
   {
   	case 1: //Facile
		return 10000; //essaie illimites
		break;
	case 2: //moyen
		return 25;
		break;
...
int main()
{
   ...
   {
     int nbr_tour;
     printf("Combien de tour voulez-vous jouer ?\n");
     scanf("%d",&nbr_tour);
     int score1,score2; 
     for(int tour=1; tour<=nbr_tour; tour++)  
     {
	printf("Round: %d\n\n",tour);
